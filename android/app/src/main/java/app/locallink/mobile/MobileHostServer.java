...
final class MobileHostServer {
    ...
    private void sendAsset(BufferedOutputStream out,String name)throws Exception{if(!(name.equals("index.html")||name.equals("style.css")||name.equals("app.js")||name.equals("qrcode.min.js"))){sendError(out,404,"未找到");return;}AssetManager assets=context.getAssets();try(InputStream in=assets.open(name);ByteArrayOutputStream data=new ByteArrayOutputStream()){byte[] buffer=new byte[16384];int read;while((read=in.read(buffer))>=0)data.write(buffer,0,read);sendText(out,200,"OK",mime(name),data.toByteArray());}}
